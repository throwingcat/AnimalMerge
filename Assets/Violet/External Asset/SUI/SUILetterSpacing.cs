...
namespace UnityEngine.UI
{
    ...
    public class SUILetterSpacing : BaseMeshEffect
    {
        ...
        public void ModifyVertices(List<UIVertex> verts)
        {
            ...
            IEnumerator
                matchedTagCollection = null; // when using RichText this will collect all tags (index, length, value)
            ...
            for (var lineIdx = 0; lineIdx < lines.Length; lineIdx++)
            {
                ...
                if (isRichText)
                {
                    ...
                    if (matchedTagCollection.MoveNext()) currentMatchedTag = (Match) matchedTagCollection.Current;
                }
                ...
                for (int charIdx = 0, actualCharIndex = 0; charIdx < line.Length; charIdx++, actualCharIndex++)
                {
                    if (isRichText)
                        if (currentMatchedTag != null && currentMatchedTag.Index == charIdx)
                        {
                            ...
                            glyphIdx += currentMatchedTag
                                .Length; // glyph index is not incremented in for loop so skip entire length
