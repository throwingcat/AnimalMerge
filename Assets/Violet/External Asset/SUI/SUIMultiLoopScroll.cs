...
namespace Violet
{
    ...
    public class SUIMultiLoopScroll : MonoBehaviour
    {
        ...
        public List<T> GetCells<T>()
        {
            var cells = new List<T>();
            foreach (var item in mCellList) cells.Add((T) item);
            return cells;
        }
        ...
        public void Init()
        {
            ...
            DefaultCellSize = mIsVertical ? cellPrefab.sizeDelta.y : cellPrefab.sizeDelta.x;
            mIsGrid = gridCount > 1;
        ...
        // 데이터 설정.
        public void SetData<T>(List<T> list) where T : IMultiScrollData
        {
            // 초기화.
            Init();
        ...
        {
            // 생성할 셀개수 계산.
            mCellCount = ((int) (mOriginRectSize / DefaultCellSize) + 2) * (mIsGrid ? gridCount : 1);
        ...
        public void RefreshContentSize()
        {
            ...
            var cellSize = 0f;
            for (var i = 0; i < mDataList.Count; i++)
                cellSize += mDataList[i].GetSize();

            // Content 크기 변경.
            var contentSize = cellSize + mCachePaddingBoth + mCacheSpacing * (cellRow - 1);
        ...
        private void UpdateCells(bool forceUpdate)
        {
            ...
            var topCellDataIndex = 0;
            var pos = currentContentPos;

            for (var i = 0; i < mDataList.Count; i++)
            {
                pos -= mDataList[i].GetSize() + mCacheSpacing;
        ...
        private void UpdateCell(int cellIndex, int topDataIndex, int topCellIndex, bool forceUpdate)
        {
            ...
            if (targetDataIndex >= mDataList.Count)
            {
                ...
                var tempPos = 0f;
                for (var i = 0; i < tempDataIndex; i++)
                    tempPos += mDataList[i].GetSize() + mCacheSpacing;
                tempPos += mCachePadding;
            ...
            for (var i = 0; i < targetDataIndex; i++)
                targetPos += mDataList[i].GetSize() + mCacheSpacing;
            targetPos += mCachePadding;
        ...
        // 데이터 목록.
        private readonly List<IMultiScrollData> mDataList = new List<IMultiScrollData>();

        #endregion
    }

    public interface IMultiScrollData : IScrollData
    {
        float GetSize();
    }
}